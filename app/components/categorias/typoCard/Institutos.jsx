export default function Institutos({datos}){
    return(
        <>
        {
            datos.results.map(e=> ( 
                <div key={e.id} className="section-card flex flex-col mb-5 shadow-slate-500 shadow-lg">
                    <div  className="flex-1 flex flex-col p-2 mt-2 h-full">
                        ...
                        <div className="position-apartado">
                                <a href={e.link} target="_black" className="asd colorMarron text-center text-sm p-2 w-2/4  text-white rounded-sm">Enlace del apartado</a>
                            </div>
                    </div>
                </div>
            ))
